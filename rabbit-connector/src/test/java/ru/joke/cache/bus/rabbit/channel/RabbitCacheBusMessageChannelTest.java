package ru.joke.cache.bus.rabbit.channel;
...
import ru.joke.cache.bus.core.impl.internal.ImmutableCacheEntryOutputMessage;
...
import ru.joke.cache.bus.rabbit.configuration.RabbitCacheBusMessageChannelConfiguration;
...
public class RabbitCacheBusMessageChannelTest {
            ...
            this.consumerArgumentCaptor.getValue().handleDelivery(null, null,
                    new AMQP.BasicProperties()
                        .builder()
                            .headers(
                                    Map.of(
                                            RabbitCacheBusMessageChannel.HOST_PROPERTY, HOST_NAME,
                                            RabbitCacheBusMessageChannel.HASH_KEY_PROPERTY, hash
                                    )
                            )
                            .type(MESSAGE_TYPE)
                            .deliveryMode(2)
    ...
    private void makeSuccessSendChecks(final CacheEntryOutputMessage outputMessage, final int publishCount) throws IOException {
        ...
        verify(this.channel, times(publishCount)).basicPublish(eq(""), eq(CHANNEL_NAME), assertArg(props -> {
            ...
            assertEquals(HOST_NAME, props.getHeaders().get(RabbitCacheBusMessageChannel.HOST_PROPERTY), "Host name must be equal");
            assertEquals(outputMessage.messageHashKey(), props.getHeaders().get(RabbitCacheBusMessageChannel.HASH_KEY_PROPERTY), "Hash key must be equal");
